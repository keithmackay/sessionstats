...
- The script writes an END row flagged `[Manual Break]` (duration/tokens computed via the same incremental mechanism as `precompact.ts`), then immediately writes a new START row with the same `session_id` and the current timestamp.
...
- Whether `[Compacted]`/`[Manual Break]` END rows get posted to the website — open forced decision, not resolved by this design (see discussion with the user).
